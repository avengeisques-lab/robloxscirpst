...
getgenv().BloxStrikeSettings = {
   ...
   -- Rage Bot (Fixed target locking & stabilization)
   RageAimbot = false,
   RageHitbox = "Head",
   RageFOV = 300,
...
RunService.RenderStepped:Connect(function()
   ...
   -- Stable Rage Aimbot Logic (Locks onto closest valid enemy inside FOV without jittery snapping)
   if getgenv().BloxStrikeSettings.RageAimbot then
      ...
      if bestTarget then
         -- Interpolate camera smoothly to stop violent flicks while maintaining target locking
...
-- Skin Changer Loop (Applies color configurations to equipped weapon meshes)
RunService.Heartbeat:Connect(function()
   ...
   for _, child in ipairs(char:GetChildren()) do
      if child:IsA("Tool") then
         local skinColor = Color3.fromRGB(150, 0, 0) -- Ruby default
