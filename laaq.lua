...
getgenv().BloxStrikeSettings = {
   -- General / Security
   TeamCheck = true,
   SkipAirborne = true, -- Ignores jumping/falling players
...
-- ==================== RAGE TAB ====================
RageTab:CreateSection("Rage Bot (Stable)")

RageTab:CreateToggle({
   Name = "Enable Rage Aimbot",
   CurrentValue = false,
   Flag = "RageToggle",
   Callback = function(Value)
...
-- ==================== VISUALS TAB ====================
VisualsTab:CreateSection("ESP & Chams Filters")
...
VisualsTab:CreateToggle({
   Name = "Skip Airborne Targets (Ignore Jump/Fall)",
   CurrentValue = true,
   Flag = "SkipAirborne",
   Callback = function(Value)
      getgenv().BloxStrikeSettings.SkipAirborne = Value
   end,
})
...
-- Function to check if target is grounded (not jumping or falling through the air)
local function isGrounded(humanoid)
   if not getgenv().BloxStrikeSettings.SkipAirborne then return true end
...
-- Render loop for Visuals, Movement, and Stable Aimbots
RunService.RenderStepped:Connect(function()
   ...
   for _, player in ipairs(Players:GetPlayers()) do
      if player ~= LocalPlayer then
         ...
         if shouldShow and targetChar and hrp and humanoid and humanoid.Health > 0 and isEnemy(player) and isGrounded(humanoid) then
   ...
   -- Stable Rage Aimbot Logic (Ignores airborne players)
   if getgenv().BloxStrikeSettings.RageAimbot then
      ...
      for _, player in ipairs(Players:GetPlayers()) do
         if player ~= LocalPlayer and isEnemy(player) then
            ...
            if hitbox and hum and hum.Health > 0 and isGrounded(hum) then
...
-- Legit Aimbot (Ignores airborne players)
...
-- Real Chams Loop via Instance Highlights (respects airborne filter)
RunService.Heartbeat:Connect(function()
   for _, player in ipairs(Players:GetPlayers()) do
      if player ~= LocalPlayer then
         local char = player.Character
         if char then
            ...
            if getgenv().BloxStrikeSettings.Chams and isEnemy(player) and (not getgenv().BloxStrikeSettings.SkipAirborne or (hum and isGrounded(hum))) then
...
RunService.Heartbeat:Connect(function()
   ...
   for _, child in ipairs(char:GetChildren()) do
      if child:IsA("Tool") then
         local skinColor = Color3.fromRGB(150, 0, 0)
