...
local Window = Rayfield:CreateWindow({
   Name = "Combat & Utility Suite [v2]",
   LoadingTitle = "Initializing Silent Systems...",
   LoadingSubtitle = "p7zu framework",
   ConfigurationSaving = {
      Enabled = true,
      FolderName = "RayfieldConfig",
      FileName = "SilentConfig"
   },
   KeySystem = false,
})
...
-- Function to get closest target within FOV circle
...
-- Hook Namecall to intercept standard Raycasts used by gun frameworks
local oldNamecall
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if getgenv().Config.SilentEnabled and not checkcaller() then
        if method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist" then
            ...
            if targetPart then
                -- Intercept and redirect ray origin direction toward target part position
                local ray = args[1]
